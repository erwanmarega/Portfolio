<template>
	<div class="relative">
	  <main :class="{'blur-background': isModalOpen}" class="min-h-screen flex items-center justify-center bg-gray-100 flex-col p-4 relative">
		...
		<div
		  ...
		  :class="isVisible ? 'translate-x-0 opacity-100' : 'translate-x-10 opacity-0'"
		>
		  <img :src="calculatrice_ios" alt="Calculatrice IOS" class="album-img sm:mr-8 mb-4 sm:mb-0" />
		  <div class="flex flex-col justify-center mt-4 sm:mt-0">
	  ...
	  <transition name="modal-fade">
		<div v-if="isModalOpen" class="modal-overlay" @click="closeModal">
		  <div class="modal-content" @click.stop>
			<img :src="modalImage" :alt="modalTitle" class="modal-img w-full max-w-screen-sm mx-auto rounded" />
  ...
  .modal-overlay {
	@apply fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-60; /* Assure que la modale est au-dessus */
  }
  
  .modal-content {
	@apply bg-white p-8 rounded-lg shadow-lg relative text-center z-60; /* Assure que la modale a un z-index supérieur */
  }
